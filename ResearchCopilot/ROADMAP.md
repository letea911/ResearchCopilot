...
- 📅 最后更新：2026-07-15
- 🔄 当前阶段：Phase 9 — 库嵌套（两级父子引用）完成 + 启动性能/稳定性修复
- ⏳ 下一里程碑：用户亲测；后续 Experiments 模块
...
| P1: Chunk 按 section 切分 + 页码溯源（引用显示 "Results, p.1"） | ✅ | 2026-07-08 |
| P1: 对话历史支持（`research chat` 多轮追问） | ✅ | 2026-07-08 |
...
| 任务 | 状态 | 日期 |
|---|---|---|
| 存储：collections 表加 parent 列 + create/list/tree/expand/rename 全升级 | ✅ | 2026-07-15 |
| 检索：hybrid 接入 expand_collections（父库勾选→自动展开为所有子库） | ✅ | 2026-07-15 |
| 分类器：prompt 两级推荐（suggested_parent/sub + new_parent/sub） | ✅ | 2026-07-15 |
| GUI：QTreeWidget 三层树 + 勾选联动 + 新建库 parent 下拉 + 嵌套导入下拉 | ✅ | 2026-07-15 |
| GUI：分类器弹窗目标库嵌套显示 + 保存解析「→」格式 | ✅ | 2026-07-15 |
...
| 日期 | 关键进展 |
|---|---|
| 2026-07-15 | 启动修复：ChromaDB 真异步 + SQLite 迁移顺序 + 模型本地优先 + run_until_complete 崩溃修 + 父库按钮/右键子库 + 分类 prompt 优化 |
| 2026-07-15 | Phase 9：库嵌套 — 两级父子引用；检索父库自动展开子库；GUI 三层树+勾选联动+嵌套下拉；分类器两级推荐 |
